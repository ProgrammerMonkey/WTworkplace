		...
		$("#button-import").upload({  
            ...
            onSelect: function () {  
                ...
                if (reg.test(this.filename())) {  
                    ...
                    $.messager.alert("警告","请选择正确的excel文件格式","warning"); 
                }  
            },  
            onComplete: function (data) {
            	var flag = eval("("+data+")");
            	if(flag){
            		  $.messager.alert("恭喜","文件导入完成","info"); 
            	}else{
            		  $.messager.alert("警告","文件导入失败","warning"); 
            	}
            }
	...
	$.extend($.fn.validatebox.defaults.rules, { 
			uniqueId: { 
				validator: function(value, param){ 
					   var flag;//  
			            $.ajax({  
		                    url : '${pageContext.request.contextPath}/regionAction_validId',  
		                    type : 'POST',                    
		                    timeout : 60000,  
		                    data:{"id" : value},  
		                    async: false,    
					   ...
					   //  双击修改  去除 手机号用户提示校验信息
			            if(!flag){  
			            	// 样式效果
		                   $("#codeId").removeClass('validatebox-invalid');  
		...
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form id="addRegionForm" action="${pageContext.request.contextPath}/regionAction_save" method="post">
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>编号</td>
						<td><input type="text" name="id" class="easyui-validatebox" required="true" id="codeId"
						data-options="validType:'uniqueId'"/></td>
					</tr>
					<tr>
						<td>省</td>
