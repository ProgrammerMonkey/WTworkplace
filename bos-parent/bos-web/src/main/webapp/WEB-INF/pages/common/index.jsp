	...
	$(function() {
		...
		$("#btnEp").click(function(){
			...
			if(regx.test(newPass)){
				$.messager.alert("警告","请不要输入空格","warning");
				return ;	
			}
			if(newPass.length<3||newPass.length>6){
				$.messager.alert("警告","请输入3-6个字符","warning");
				return ;
			}
			if(newPass!=RePass){
			...
			$.post("${pageContext.request.contextPath}/user/userAction_edit",{"password":newPass},function(data){
				if(data){
					$.messager.alert("成功","密码修改成功","info");
					$("#editPwdWindow").window('close');
				}else{
					$.messager.alert("失败","密码修改失败","error");
				}
			});
		});
	});
...
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="sessionInfoDiv"
			...
			[<strong>${sessionScope.exitsUser.username }</strong>]，欢迎你！您使用[<strong>${pageContext.request.remoteAddr }</strong>]IP登录！
		</div>
